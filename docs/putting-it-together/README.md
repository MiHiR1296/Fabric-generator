...
- **Phase 5 — uniform-aspect U + per-strand spool + curve-normal symmetry** (2026-05-16): ✓ done. Texture is no longer squished — per-yarn `Material N Texture Scale U` is auto-derived from the detected core V band and the graph's core fraction, and the new `U Stride Per Warp End / Weft Pick` sockets spool one continuous yarn across strands (strand `N+1` starts where strand `N` ended). The Phase 4d/4i fit-calibration chain retired; `UV Random U` default flipped to `0.0` since stride provides natural variation. Cross-axis V-padding asymmetry fixed by inserting `Set Curve Normal (Z Up)` on both warp and weft branches. See [../BlenderFixes/phase_log.md](../BlenderFixes/phase_log.md) Phase 5 for the .blend changes and [phase_log.md](phase_log.md) Phase 5 for the backend changes.
- **Phase 10u / Checkpoint 2026-05-19 — approved Blender/material baseline**: ✓ done. The checkpoint is [../CHECKPOINT_2026-05-19.md](../CHECKPOINT_2026-05-19.md): `Spacing = 0.026`, `Arc 1 V Padding = 0.008`, `Texture Offset V = 0`, `Sub Texture Scale V = 1`, `Sub Texture Offset V = 0`, and root `Texture Scale U = 1`. The current auto U path derives `Material N Texture Scale U` from the material core band times the graph core fraction.
- **2026-06-05 material/padding update**: ✓ done. Generated RGBA yarn materials now use `FabricStudioDiffuseNode.Alpha` directly, with no default alpha texture/remap node on the RGBA path. `Arc 1 V Padding` now defaults to `0.02` in backend and frontend render settings.
- **Publish prep**: ✓ done. Local debug/backups ignored, raw `big-lama.pt` can be supplied by `BIG_LAMA_MODEL_PATH` / `LAMA_MODEL` or reconstructed from tracked chunks with `make backend-setup-lama`, GitHub's current LFS-disabled state is documented, and [../FINAL_HANDOFF.md](../FINAL_HANDOFF.md) summarizes the final application.
