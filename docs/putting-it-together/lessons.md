...
The automatic `Material N Texture Scale U` value must be derived from the material's detected core band, not from a root `Texture Scale U` tweak and not from `Arc 1 V Padding`. Padding widens the rendered V band, but it is a visual expansion control; letting it change U makes the same yarn consume a different source length when only the halo width changes. The current auto formula is `material_texture_scale_u = (core_v_max - core_v_min) * AUTO_TEXTURE_SCALE_U_CORE_FRACTION`, where `AUTO_TEXTURE_SCALE_U_CORE_FRACTION = ARC1_V_AROUND_SPAN = 0.6`.
...
**How to apply**: any future change to the V-band mapping (e.g. moving the `ARC1_V_AROUND_SPAN` constant, changing core-band semantics, or adding a new Arc 1/Arc 2 match mode) must be paired with a matching update to the per-yarn U scale and the stride derivation in [blender_live.py](../../backend/app/blender_live.py). Producers should ship `bandMeta.blender.texture_scale_u = 1.0` (or omit) — the consumer auto-derives the correct value from the material core band. A producer who ships a non-`1.0` override is asserting "I know better" and the consumer respects it.
