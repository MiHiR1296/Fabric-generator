...
## Current state (2026-05-19 visual checkpoint)
...
- **Current Arc 2 free-flow halo state (Phase 10t)**: the live saved `.blend` uses the real Arc 2 Top/Core/Bot path, with `Arc 2 Match Arc 1 V Rate = false`, `Arc 2 Edge Angle Mapping = false`, and `Match Section Slopes = false`. Arc 2's center interval is still the R1-equivalent core (`Split Minus/Plus = 0.2/0.8` from `r1/r2 = 0.015/0.025 = 0.6`), but the two outer R2 intervals now extrapolate from the raw core seam at the core V rate instead of snapping to `Arc 2 V Min/Max`. Material 1 evaluated readback: Arc 2 `v_around 0.0/0.2/0.5/0.8/1.0 -> uv_y 0.503224/0.514507/0.531431/0.548356/0.559639`, with ratio `1.0` in Top/Core/Bot.
...
- **Arc 2 same-strand U transfer (Phase 10k)**: active. Phase 10j's `PW VisU - *` projected-axis correction and the first ungrouped nearest-surface transfer both distorted because they sampled the wrong visual model / wrong neighboring strands. The saved graph now writes internal `pw_strand_id` values on warp and weft curves before the join, then Arc 2 uses `Sample Nearest Surface` with matching `Group ID` / `Sample Group ID` to borrow only Arc 1's U from the same physical strand. `Combine XYZ.006.X` is fed by `PW StrandXferU - Use Same-Strand Arc1 U On Arc2`; Arc 2 keeps its own V mapping. Current active graph has no `PW VisU` nodes and no `pw_visual_u_correction`, `pw_u_factor`, or `pw_thread_kind` attributes.
...
- **Match Section Slopes (Phase 10e)**: Surface-panel boolean toggle, default `False`. When on, replaces Phase 3q's fixed geometric split (`arc1/arc2 = 0.6 → 0.2 / 0.8`) with a texture-proportional split derived from Material 1's V values. All four section slopes become equal → eliminates the V-slope-discontinuity band at section boundaries. Caveat: multi-material renders only match exactly for Material 1.
- **Arc 2 Edge Angle Mapping (Phase 10f)**: Surface-panel boolean toggle, default `False`. When on, replaces the linear-in-`v_around` mapping in Top/Bot sections with a cumulative-`sin(θ)` mapping (= linear-in-`cos(θ)`), where `θ` is the actual angle of curvature on the profile arc. Compensates for the silhouette polygons projecting to small screen footprint by allocating less V there — cells at the silhouette shrink to match Core cell sizes. Core stays linear. Trade-off: introduces a slope discontinuity at the Top↔Core / Core↔Bot section boundary (cosine peak slope ≠ linear Core slope). Usually mitigated more simply by lowering `Arc 1 V Padding` so halo spans are wide enough to keep linear slopes naturally aligned.
...
- **Recommended baseline for clean renders**:
  ...
  - Current approved free-flow halo mode: `Spacing = 0.026`, `Arc 1 V Padding = 0.008`, `Match Section Slopes = false`, `Arc 2 Match Arc 1 V Rate = false`, `Arc 2 Edge Angle Mapping = false`, `Arc 2 Boundary Inset = 0.0`, `Texture Offset V = 0`, with Phase 10k same-strand Arc 2 U transfer, Phase 10t free halo V, and Phase 10u U denominator active. This keeps the central R1-equivalent core interval at the same scale model as Arc 1 and lets the two outer Arc 2 strips continue that V rate outward.
- **Pinned / neutral**: `Sub Strand Enable = True` (Socket_84) so Arc 2 halo always renders; the V footgun sockets are pinned at the approved checkpoint defaults (`Texture Scale V = 1`, `Texture Offset V = 0`, `Texture Side Flatten = 0`, `Sub Texture Scale V = 1`, `Sub Texture Offset V = 0`). Root `Texture Scale U` is neutral (`1.0`); use it only as a temporary visual test knob. In the Phase 10u denominator-test state, per-yarn U scale comes from `Material N Texture Scale U`; when the producer leaves `texture_scale_u` unset/`1.0`, the apply step derives it from the current padded visible span divided by `AUTO_TEXTURE_SCALE_U_DENOMINATOR = 1.0`. Explicit non-`1.0` producer overrides are still respected.
